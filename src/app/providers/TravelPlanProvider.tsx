...
interface TravelPlanContextValue { plan: TravelPlan | null; setPlan: (plan: TravelPlan | null) => void; addSpot: (spot: Spot) => void; removeSpot: (id: string) => void; moveSpot: (id: string, direction: -1 | 1) => void; setTransport: (index: number, mode: TransportMode) => void; setRoute: (index: number, route: RouteSummary | null) => void; save: () => Promise<void>; }
...
export function TravelPlanProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<TravelPlanContextValue>(() => ({
    plan, setPlan,
    addSpot: (spot) => setPlan((current) => current ? { ...current, spots: [...current.spots, { spot, visitOrder: current.spots.length + 1, transportMode: current.spots.length ? 'DRIVING' : undefined }], routes: current.spots.length ? [...current.routes, null] : current.routes } : current),
